...
public class Generator {
    ...
    public void generate(String dataSourceurl, String dataSourcename, String dataSourcepassword, String dataSourcedriver, String tables, String packageParent, boolean isNormalize) {
        ...
        String projectPath ="D:\\workspace\\hermes_project";// 当前项目的路径
        gc.setOutputDir(projectPath + "\\hermes-web\\src\\main\\java");// 生成文件输出根目录
        gc.setAuthor("jiaoyun");// 作者
        gc.setOpen(false); // 生成完成后不弹出文件框
        gc.setFileOverride(true); // 文件是否覆盖
        gc.setIdType(IdType.UUID); //主键策略 实体类主键ID类型
        gc.setDateType(DateType.ONLY_DATE);
        gc.setSwagger2(true); // 是否开启swagger
        gc.setActiveRecord(false); //【不懂】 活动记录 不需要ActiveRecord特性的请改为false 是否支持AR模式
        ...
        pc.setModuleName("lujing"); // 控制层请求地址的包名显示
        ...
        if (true == isNormalize) {
            ...
            // 自动填充配置
            TableFill gmtCreate = new TableFill("CREATE_NAME", FieldFill.INSERT);
            TableFill gmtModified = new TableFill("MODIFY_TIME",
